...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upsert fields in fund/crm/deals.json")
    parser.add_argument("--file", default="fund/crm/deals.json")
    ...
    company = next((item for item in registry["companies"] if item["slug"] == args.slug), None)
    if company is None:
        raise SystemExit(f"Company slug not found: {args.slug}")

    for item in args.set:
        key, raw_value = item.split("=", 1)
        set_path(company, key, parse_value(raw_value))

    for item in args.append_list:
        key, raw_value = item.split("=", 1)
        append_path(company, key, parse_value(raw_value))
